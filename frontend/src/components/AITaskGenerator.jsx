...
export default function AITaskGenerator({ onTasksCreated }) {
  ...
  return (
    <div className="sketch-hover border-2 border-gray-900 rounded-lg relative overflow-hidden">
      ...
      <div className="px-6 py-5 border-b-2 border-gray-900">
        ...
        <p className="text-xs text-gray-400 mt-0.5">Describe what you want to accomplish</p>
      </div>
      ...
      <div className="px-6 py-5">
          ...
          <form onSubmit={handleSaveApiKey}>
            <p className="text-sm text-gray-600 mb-3">
              ...
              <a href="https://aistudio.google.com/apikey" target="_blank" rel="noreferrer"
                className="font-bold text-gray-900 underline">Google AI Studio</a>.
            </p>
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-3 py-2 border-2 border-gray-300 rounded-lg text-sm focus:border-gray-900 focus:ring-0 outline-none"
              />
              <button
                type="submit"
                className="px-4 py-2 bg-gray-900 text-white font-bold text-sm rounded-lg border-2 border-gray-900 hover:bg-gray-800"
              >
                Save
              </button>
            </div>
        ...
        {step === 'idle' && (
          <form onSubmit={handleGenerate} className="flex gap-2">
            <input
              ...
              className="flex-1 px-3 py-2.5 border-2 border-gray-300 rounded-lg text-sm focus:border-gray-900 focus:ring-0 outline-none"
            />
            <button
              type="submit"
              disabled={!goal.trim()}
              className="px-5 py-2.5 bg-gray-900 text-white font-bold text-sm rounded-lg border-2 border-gray-900
        ...
        {step === 'review' && (
          <div>
            <p className="text-sm font-bold text-gray-900 mb-1">{summary}</p>
            <p className="text-xs text-gray-400 mb-4">{tasks.length} tasks generated</p>

            <div className="space-y-2 mb-5">
              {tasks.map((task, i) => (
                <div key={i} className="border-2 border-gray-200 rounded-lg p-3 flex items-start gap-3">
                  <span className="text-xs font-mono font-bold text-gray-300 mt-0.5">{i + 1}</span>
                  <div className="flex-1 min-w-0">
                    ...
                    {task.category && (
                      <span className="inline-block mt-1 text-xs font-bold bg-white border border-gray-300 text-gray-500 px-2 py-0.5 rounded">
                        {task.category}
                      </span>
                    )}
                  </div>
            ...
            <div className="flex gap-3 mb-4">
              ...
              <button
                ...
                className="px-4 py-2.5 border-2 border-gray-300 text-gray-500 font-bold text-sm rounded-lg
            ...
            <form onSubmit={handleRefine} className="flex gap-2">
              ...
              <button
                ...
                className="px-4 py-2 border-2 border-gray-900 text-gray-900 font-bold text-sm rounded-lg
                           hover:bg-gray-900 hover:text-white disabled:opacity-40 transition-colors"
              >
                Refine
              </button>
            </form>
        ...
        {step === 'done' && (
          <div className="py-2">
            ...
            <p className="text-xs text-gray-400 mb-4">Head to the Board to manage them.</p>
            <button
              onClick={handleReset}
              className="px-4 py-2 border-2 border-gray-900 text-gray-900 font-bold text-sm rounded-lg
